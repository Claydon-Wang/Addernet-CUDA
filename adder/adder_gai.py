...
def get_conv2d_output_shape(input, weight, stride, padding):
    ...
    padding_h = int(padding[0])
    padding_w = int(padding[1])
...
class Adder2DFunction(torch.autograd.Function):
    ...
    def forward(ctx,
        ...
        kernel_size_h = int(kernel_size[0])
        kernel_size_w = int(kernel_size[1])
        stride_h = int(stride[0])
        stride_w = int(stride[1])
        padding_h = int(padding[0])
        padding_w = int(padding[1])
    ...
    def __init__(self,
        ...
        self.adder = torch.nn.Parameter(
            nn.init.normal_(torch.randn(
                output_channel, input_channel, int(kernel_size[0]), int(kernel_size[1]))))
        self.qadder = None
        self.bias = bias
        if bias:
